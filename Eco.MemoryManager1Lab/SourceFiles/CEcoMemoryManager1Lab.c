...
static bool_t ECOCALLMETHOD CEcoMemoryManager1Lab_623E1838_get_Status(/* in */ IEcoMemoryManager1Ptr_t me, /* in | out */ ECOMEMORYMANAGER1STATUS* status) {
    ...
    /* Подсчет использованных блоков и размера */
    uUsedBlocks = 0;
    uAllocSize = 0;
    pChunk = pCMe->m_pChunkList;
    while (pChunk != 0) {
        if (pChunk->bInUse != 0) {
            uUsedBlocks++;
            uAllocSize += (uint32_t)(pChunk->pEnd - pChunk->pBegin);
        }
        pChunk = pChunk->pNext;
    }
    
    uTotalSize = (uint32_t)(pCMe->m_pMemLimit - pCMe->m_pMemBase);

    status->lowAddr = (uint32_t)((uint64_t)pCMe->m_pMemBase);
    status->highAddr = (uint32_t)((uint64_t)pCMe->m_pMemBase >> 32);
    status->totalSize = uTotalSize;
    status->freeSize = uTotalSize - uAllocSize;
    status->usedBlocks = uUsedBlocks;
