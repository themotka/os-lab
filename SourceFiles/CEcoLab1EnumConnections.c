...
int16_t ECOCALLMETHOD CEcoLab1EnumConnections_Next(/* in */ struct IEcoEnumConnections* me, /* in */ uint32_t cConnections, /* out */ struct EcoConnectionData *rgcd, /* out */ uint32_t *pcFetched) {
    ...
    if (pCMe->m_pSinkList != 0) {
        maxCount = pCMe->m_pSinkList->pVTbl->Count(pCMe->m_pSinkList);
        while ((pCMe->m_cIndex < maxCount) && (count < cConnections)) {
        ...
        maxCount = pCMe->m_cConnectionsCount;
        while ((pCMe->m_cIndex < maxCount) && (count < cConnections)) {
            pCD = pCMe->m_pConnections[pCMe->m_cIndex];
            if (pCD != 0) {
                pCD->pUnk->pVTbl->AddRef(pCD->pUnk);
                rgcd->pUnk = pCD->pUnk;
                rgcd->cCookie = pCD->cCookie;
 ...
 *
 * <сводка>
 *   Функция Create From Array
 * </сводка>
 *
 * <описание>
 *   Функция создания экземпляра из массива подключений
 * </описание>
 *
 */
int16_t ECOCALLMETHOD createCEcoLab1EnumConnectionsFromArray(/* in */ IEcoUnknown* pIUnkSystem, /* in */ EcoConnectionData** pConnections, /* in */ uint32_t cCount, /* out */ IEcoEnumConnections** ppIEnum) {
    ...
    /* Проверка указателей */
    if (ppIEnum == 0 || pIUnkSystem == 0 || pConnections == 0) {
        return result;
    }
    ...
    /* Сохранение массива подключений */
    pCMe->m_pSinkList = 0;
    pCMe->m_pConnections = (EcoConnectionData**)pIMem->pVTbl->Alloc(pIMem, sizeof(EcoConnectionData*) * cCount);
    if (pCMe->m_pConnections == 0) {
        deleteCEcoLab1EnumConnections((IEcoEnumConnections*)pCMe);
        return -1;
    }
    pCMe->m_cConnectionsCount = cCount;
    for (indx = 0; indx < cCount; indx++) {
        pCD = pConnections[indx];
        if (pCD != 0) {
            pNewCD = (EcoConnectionData*)pIMem->pVTbl->Alloc(pIMem, sizeof(EcoConnectionData));
            pNewCD->cCookie = pCD->cCookie;
            pNewCD->pUnk = pCD->pUnk;
            pNewCD->pUnk->pVTbl->AddRef(pNewCD->pUnk);
            pCMe->m_pConnections[indx] = pNewCD;
        } else {
            pCMe->m_pConnections[indx] = 0;
        }
    }
    pCMe->m_cIndex = 0;
