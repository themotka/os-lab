...
int16_t ECOCALLMETHOD CEcoLab1ConnectionPoint_Advise(/* in */ struct IEcoConnectionPoint* me, /* in */ struct IEcoUnknown *pUnkSink, /* out */ uint32_t *pcCookie) {
    ...
    if (result == 0 && pCD->pUnk != 0) {
        ...
        if (pCMe->m_pSinkList != 0) {
            ...
            /* Добавление в массив */
            if (pCMe->m_cConnectionsCount >= pCMe->m_cConnectionsCapacity) {
                /* Увеличение размера массива */
                uint32_t newCapacity = pCMe->m_cConnectionsCapacity * 2;
                EcoConnectionData** pNew = (EcoConnectionData**)pCMe->m_pIMem->pVTbl->Alloc(pCMe->m_pIMem, sizeof(EcoConnectionData*) * newCapacity);
                if (pNew == 0) {
                    pCMe->m_pIMem->pVTbl->Free(pCMe->m_pIMem, pCD);
                    return -1;
                }
                for (i = 0; i < pCMe->m_cConnectionsCount; i++) {
                    pNew[i] = pCMe->m_pConnections[i];
                }
                for (i = pCMe->m_cConnectionsCount; i < newCapacity; i++) {
                    pNew[i] = 0;
                }
                pCMe->m_pIMem->pVTbl->Free(pCMe->m_pIMem, pCMe->m_pConnections);
                pCMe->m_pConnections = pNew;
                pCMe->m_cConnectionsCapacity = newCapacity;
            }
            pCMe->m_pConnections[pCMe->m_cConnectionsCount] = pCD;
            pCMe->m_cConnectionsCount++;
        }
...
int16_t ECOCALLMETHOD CEcoLab1ConnectionPoint_Unadvise(/* in */ struct IEcoConnectionPoint* me, /* in */ uint32_t cCookie) {
    ...
    if (pCMe->m_pSinkList != 0) {
        ...
        /* Поиск в массиве */
        for (indx = 0; indx < pCMe->m_cConnectionsCount; indx++) {
            pCD = pCMe->m_pConnections[indx];
            if (pCD != 0 && pCD->cCookie == cCookie) {
                pSink = pCD->pUnk;

                /* Удаление из массива - сдвигаем элементы */
                {
                    uint32_t j;
                    for (j = indx; j < pCMe->m_cConnectionsCount - 1; j++) {
                        pCMe->m_pConnections[j] = pCMe->m_pConnections[j + 1];
                    }
                    pCMe->m_pConnections[pCMe->m_cConnectionsCount - 1] = 0;
                    pCMe->m_cConnectionsCount--;
                }
                pSink->pVTbl->Release(pSink);
                pCMe->m_pIMem->pVTbl->Free(pCMe->m_pIMem, pCD);
...
int16_t ECOCALLMETHOD CEcoLab1ConnectionPoint_EnumConnections(/* in */ struct IEcoConnectionPoint* me, /* out */ struct IEcoEnumConnections **ppEnum) {
    ...
    if (pCMe->m_pSinkList != 0) {
        /* Создание нумератора подключений из списка */
        return createCEcoLab1EnumConnections((IEcoUnknown*)pCMe->m_pISys, pCMe->m_pSinkList, ppEnum);
    } else if (pCMe->m_pConnections != 0) {
        /* Создание нумератора подключений из массива */
        return createCEcoLab1EnumConnectionsFromArray((IEcoUnknown*)pCMe->m_pISys, pCMe->m_pConnections, pCMe->m_cConnectionsCount, ppEnum);
    }
    /* Если нет ни списка, ни массива, возвращаем ошибку */
    return -1;
}
...
int16_t ECOCALLMETHOD createCEcoLab1ConnectionPoint(/* in */ IEcoUnknown* pIUnkSystem, /* in */ IEcoConnectionPointContainer* pICPC, /* in */ const UGUID* riid, /* out */ IEcoConnectionPoint** ppICP) {
    ...
    pCMe->m_pSinkList = 0;
    pCMe->m_pConnections = 0;
    pCMe->m_cConnectionsCount = 0;
    pCMe->m_cConnectionsCapacity = 0;
    result = pIBus->pVTbl->QueryComponent(pIBus, &CID_EcoList1, 0, &IID_IEcoList1, (void**)&pCMe->m_pSinkList);
    if (result != 0 || pCMe->m_pSinkList == 0) {
        /* Используем простой массив вместо списка */
        pCMe->m_cConnectionsCapacity = 4;
        pCMe->m_pConnections = (EcoConnectionData**)pIMem->pVTbl->Alloc(pIMem, sizeof(EcoConnectionData*) * pCMe->m_cConnectionsCapacity);
        if (pCMe->m_pConnections == 0) {
        ...
        {
            uint32_t i;
            for (i = 0; i < pCMe->m_cConnectionsCapacity; i++) {
                pCMe->m_pConnections[i] = 0;
            }
        }
    }
...
void ECOCALLMETHOD deleteCEcoLab1ConnectionPoint(/* in */ IEcoConnectionPoint* pICP) {
    ...
    if (pICP != 0 ) {
        ...
        if (pCMe->m_pSinkList != 0) {
            ...
            /* Освобождение массива подключений */
            for (index = 0; index < pCMe->m_cConnectionsCount; index++) {
                pCD = pCMe->m_pConnections[index];
                if (pCD != 0) {
                    if (pCD->pUnk != 0) {
            ...
            pIMem->pVTbl->Free(pIMem, pCMe->m_pConnections);
            pCMe->m_pConnections = 0;
        }
        if (pCMe->m_pISys != 0) {
